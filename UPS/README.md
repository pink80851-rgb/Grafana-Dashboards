...
- **資料來源 (Data Source)**  
  - [Zabbix](https://www.zabbix.com/) 作為後端監控資料平台。

- **可視化 (Visualization)**  
  - [Grafana](https://grafana.com/) 作為 Dashboard 平台，整合 Zabbix 資料源。
...
| Panel 名稱      | 類型        | 功能描述 |
|-----------------|------------|---------|
| 電池容量        | Bar Gauge  | 顯示 UPS 當前電池剩餘容量百分比 |
| 輸出電壓        | Stat       | 顯示 UPS 的輸出電壓 (V) |
| 可運行時間      | Stat       | 顯示 UPS 在當前負載下的估計運行時間 |
| 電池負載        | Gauge      | 顯示 UPS 負載百分比，超過閾值會標紅 |
| 輸入電壓        | Stat       | 顯示 UPS 的輸入電壓 (V) |
| 溫度            | Stat       | 顯示 UPS 溫度 (°C)，超過閾值顯示紅色 |
...
- Dashboard 架構模板化，方便快速複製或延伸至其他設備。
